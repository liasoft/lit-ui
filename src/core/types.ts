 ...
 * them to a component. This mirrors how the Froxlor panel composes its UI from
 * PHP arrays, but typed.
 */
...
/**
 * How a cell's value should be rendered. Mirrors Froxlor's twig table macros.
...
export interface TableListing<Row = Record<string, unknown>> {
  ...
  /** Row identity for selection; defaults to the array index. */
...
export type FieldType =
  ...
  | 'radio'
  | 'switch'
  /** Number input with an "unlimited" (-1) toggle — Froxlor's `textul`. */
  | 'textul'
...
/** The input of `<lia-form>` / `<lia-form-page>`. */
...
/** A resource meter tile — Froxlor's dashboard `ditem`. */
