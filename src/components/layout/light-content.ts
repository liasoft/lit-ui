...
export class LightContent {
  ...
  /**
   * @param tagName  The holder's element. `display: contents` takes it out of layout either way, so
   *   this is about where the projection is *allowed* to sit: a `div` inside an inline trigger — an
   *   anchor, a button — is flow content inside phrasing content, which is not a document a parser
   *   has to keep in one piece. An inline holder is, so an inline consumer asks for `span`.
   */
  constructor(host: HTMLElement, className = 'lia-content-slot', tagName = 'div') {
    this.host = host;
    this.holder = document.createElement(tagName);
