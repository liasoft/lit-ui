...
/**
 * Last say over what a rendered cell becomes.
 *
 * Installed once per application with {@link setCellDecorator} and consulted for **every** cell of
 * every table, after the column has had its say. It receives what the column produced and returns
 * what is shown, so it can wrap the content, replace it, or hand it straight back.
 *
 * This exists so a cross-cutting cell behaviour does not have to be spelled out column by column. An
 * application that can say something about a *kind* of value — an identifier it can resolve to a name,
 * a code it can explain, a reference it can link — otherwise has to remember to say it at every
 * listing that happens to show one, and the ones it forgets are invisible. A decorator is declared in
 * terms of the column and row, so it reaches the tables nobody thought about, including the ones
 * written later.
 *
 * Keep it cheap and total: it runs per cell per render, and a throw takes the whole table with it.
 */
export interface CellDecoratorContext {
  /** The row the cell belongs to. */
  row: unknown;
  /** The column definition, including any consumer-specific hints such as `lookup`. */
  column: TableColumn<Record<string, unknown>>;
  /** The raw value behind `column.key`, before any rendering. */
  value: unknown;
}

export type CellDecorator = (content: unknown, context: CellDecoratorContext) => unknown;

let cellDecorator: CellDecorator | null = null;

/** Install the decorator, or pass `null` to remove it. Later calls replace earlier ones. */
export function setCellDecorator(decorator: CellDecorator | null): void {
 ...
 * built-ins and `text` is the fallback. Whatever comes out then passes through
 * the installed {@link CellDecorator}, if there is one.
 *
 * @example
 * ```ts
...
export function renderCell<Row = Record<string, unknown>>(
  ...
  const content = column.render
    ? (column.render(row, column) ?? nothing)
    : renderCellValue(cellValue(row, column), column.renderer ?? 'text', {
        ...context,
        column,
        row,
      });

  if (!cellDecorator) return content;

  return cellDecorator(content, {
    row,
    column: column as unknown as TableColumn<Record<string, unknown>>,
