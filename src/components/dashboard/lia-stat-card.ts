...
import type { IconName, StatCardData, StatTone, Variant } from '../../core/types.js';
...
export class LiaStatCard extends LiaElement {
  ...
  @property({ type: String }) variant: StatTone = 'off';
  ...
  override render(): TemplateResult {
    ...
    // `off` is not a Bootstrap contextual colour, so it cannot go through `bg-*-subtle`; it is the
    // neutral chip the stylesheet defines, and it is what a card gets when it says nothing.
    const tone = variant ?? 'off';
    const chipTone = tone === 'off' ? ['lia-stat-chip-off'] : [`bg-${tone}-subtle`, `text-${tone}-emphasis`];
