...
/**
 * A full-bleed notification strip docked above the navbar: a short message on
 * the left, one or two actions on the right, no rounded corners and no bottom
 * margin so it butts straight against the navigation.
 *
 * @example
 * ```ts
 * html`<lia-banner
