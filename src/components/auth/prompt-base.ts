 ...
 *
 * The reference panel rendered each of these as a full interstitial page built
 * around an `alert alert-warning`, which meant losing the page the user was
 * on. Here the default is a real Bootstrap modal (Esc, backdrop and focus
 * handling included, torn down properly when the host is removed mid-flight),
 * with the original inline treatment still available behind the `inline` flag
 * for flows that genuinely want their own page.
 *
...
export abstract class LiaAuthPrompt<TResult> extends LiaElement {
  ...
  /** The question, rendered as trusted HTML — the twig templates used `|raw`. */
