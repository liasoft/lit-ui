...
export interface AuthFeedbackOptions extends AuthFeedbackLabels {
  /** Render the messages as trusted HTML. */
  trustedHtml?: boolean;
...
/**
 * One labelled `<input>` row, Bootstrap-styled, with hint and error slots and
 * the matching `aria-describedby` / `aria-invalid` wiring.
 */
...
export interface AuthSwitchOptions {
  ...
  checked: boolean;
  disabled?: boolean;
  /** `switch` renders a Bootstrap switch; `checkbox` a plain checkbox. */
...
/** The primary submit button of an auth card. */
...
/** A `←`-prefixed "back to sign in" link, as used by the password-recovery cards. */
 ...
 *
 * The `autofocus` attribute is no substitute: browsers only honour it for
 * markup present at parse time, not for a component that renders later.
 */
