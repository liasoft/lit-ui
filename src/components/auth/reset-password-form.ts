 ...
 *
 * The twig original had the two fields and no feedback whatsoever — a typo in
 * the confirmation was only discovered after a round trip. Here the mismatch
 * is caught on blur, the strength meter scores locally (nothing leaves the
 * page), and `min-score` decides how strong is strong enough. Set it to `0` to
 * make the meter purely advisory.
 *
 * @example
 * ```html
