 ...
 * should look.
 *
 * The reference panel split this across three sibling `<form>` cards, each
 * posting on its own — that shape is kept, because a password change and a
 * language change genuinely are separate transactions, but each card now
 * reports through its own event instead of a page post. The details card is
 * driven by an ordinary {@link FormDefinition} and rendered by `<lia-form>`,
 * so it inherits the whole form system; the password card adds the strength
 * meter and the match check the original lacked; the preferences card carries
