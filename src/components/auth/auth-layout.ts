 ...
 *
 * Every login-area twig in the reference panel repeated the same wrapper —
 * `container > row.justify-content-center > .col-12.max-w-420` with a logo
 * above a `.card.shadow` — and each one re-implemented the alert cascade by
 * hand. This element owns all of it: the vertical centring, the brand block,
 * the heading, the alert region, the card-footer link row and the colour
 * scheme switch that the original login pages did not have at all.
 *
