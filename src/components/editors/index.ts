 ...
 *
 * Where the other families render *any* listing or *any* form, these are
 * screens with a shape of their own: the DNS record table, the log dump, the
 * traffic page, the error report. Each one is built as the interaction
 * pattern it really is, free of domain semantics.
 *
 * | Element                 | The pattern                                                  |
 * | ----------------------- | ------------------------------------------------------------ |
 ...
 *
 * `<lia-code-block>` is the shared substrate: the log viewer and the diff
 * view compose it rather than re-implementing a `<pre>`.
 *
