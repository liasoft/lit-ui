...
export class LiaCard extends LiaElement {
  ...
  /**
   * A light second line under the title.
   *
   * What the card is, or - just as usefully - the conditions its content was produced under: the
   * aggregate a control chose, the bucket width, the limit that truncated it. Under the heading
   * rather than opposite it, because it is read together with the title and not as a separate fact.
   */
  ...
  /**
   * Affordances belonging to the card's content, rendered in the header before {@link headerActions}.
   *
   * Set by a child that has controls of its own and is rendering *as* this card's body - see
   * `<lia-table>`'s attached mode. A card should not carry a second header strip inside its body, so
   * the child hands its controls up rather than drawing them a few pixels below these.
   */
  @property({ attribute: false }) headerTools?: unknown;
  ...
  private get hasHeader(): boolean {
    return Boolean(
      this.title || this.icon || this.subtitle || this.headerTools || this.headerActions.length
    );
  }
