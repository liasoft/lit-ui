 ...
 * | `<lia-form-page>`     | heading + back action + alerts + note + form          |
 * | `<lia-form-note>`     | the explanatory panel above a form                    |
 * | `<lia-replacer-hint>` | the `{placeholder}` reference table                   |
 *
 * Supporting modules:
 *
 ...
 * against the live value model. That is the whole of what the reference panel
 * spent three jQuery modules on — hiding the spam thresholds behind a "bypass"
 * switch, folding away the HTTP sections of a mail-only entry, revealing the
 * advanced half of an installer — expressed in the definition instead of in
 * DOM-poking event handlers. See {@link evaluateCondition}.
 *
 * ### Asynchronous fields
 *
 * The other two things that panel hand-wrote per screen are declarative too:
 * `asyncRules` for a debounced server-side check (its IP/port collision probe)
 * and `dependsOn` + `optionsFor` / `optionsLoader` for a select narrowed by
 * another field (its "PHP configuration for this customer" list). `<lia-form>`
...
export * from './form.js';
export * from './form-note.js';
export * from './replacer-hint.js';
export * from './form-page.js';
