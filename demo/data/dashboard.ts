 ...
 *
 * Chart series reuse the kit's own validated categorical palette
 * (`USAGE_PALETTE_LIGHT` / `USAGE_PALETTE_DARK` from the editors family) rather
 * than inventing colours: those two ramps are already checked for adjacent-pair
 * separation under normal and colour-deficient vision, in both themes. Series
 * identity never rests on colour alone here either — every chart in the demo is
 * paired with a legend and a table.
 */
...
import { USAGE_PALETTE_DARK, USAGE_PALETTE_LIGHT } from '@liasoft/lit-ui';
...
/**
 * The meter tiles of `<lia-dashboard-grid>`: a limit, what is used of it, and
 * — for the tiles that hand capacity down — how much is already assigned.
 *
 * A negative `available` means unlimited, which is what the last two tiles
 * demonstrate.
 *
 * @example
 * ```ts
 * html`<lia-dashboard-grid .items=${resourceItems} columns="5"></lia-dashboard-grid>`;
 * ```
 */
export const resourceItems: ResourceItem[] = [
  { label: 'Members', available: 80, used: 60, assigned: 72, href: '#/crud/list' },
  { label: 'Services', available: 40, used: 31, assigned: 36 },
  { label: 'Volumes', available: 25, used: 24, assigned: 25 },
  {
    label: 'Storage',
    available: 2048 * GIB,
    used: 1655 * GIB,
  ...
  { label: 'Scheduled jobs', available: -1, used: 18 },
  { label: 'Access keys', available: -1, used: 7 },
  { label: 'Certificates', available: 50, used: 47, assigned: 50 },
];

/** A shorter set, for examples that must not dominate the page. */
export const resourceItemsCompact: ResourceItem[] = resourceItems.slice(0, 4);
...
/** Entries of `<lia-newsfeed>` when it is fed directly instead of from a URL. */
export const newsItems: NewsfeedItem[] = [
  {
    title: 'Console 4.12 — scheduled maintenance windows',
    link: 'https://example.org/blog/4-12',
    date: '2026-06-28',
    description:
      'Maintenance windows can now be declared per environment, and the status page picks them up automatically.',
  },
  {
    title: 'Deprecating the v1 metrics endpoint',
    link: 'https://example.org/blog/metrics-v1-sunset',
    date: '2026-06-11',
    description: 'The v1 endpoint stops answering on 1 October. The v2 shape is documented here.',
  },
  {
    title: 'Passkeys are out of preview',
    link: 'https://example.org/blog/passkeys-ga',
    date: '2026-05-30',
    description: 'Members can now register a passkey as their second factor, or as their only one.',
  },
  {
    title: 'Post-mortem: 12 May edge cache incident',
    link: 'https://example.org/blog/postmortem-2026-05-12',
    date: '2026-05-15',
  },
];
  ...
  datasets: [
    {
      label: 'Inbound (TiB)',
      data: inbound,
      backgroundColor: USAGE_PALETTE_LIGHT[0],
      borderColor: USAGE_PALETTE_LIGHT[0],
      borderWidth: 1,
    },
    {
      label: 'Outbound (TiB)',
      data: outbound,
      backgroundColor: USAGE_PALETTE_LIGHT[1],
      borderColor: USAGE_PALETTE_LIGHT[1],
      borderWidth: 1,
    },
  ],
};
...
/** A composition chart — where the storage in {@link resourceItems} sits. */
  ...
  datasets: [
    {
      ...
      backgroundColor: [
        USAGE_PALETTE_LIGHT[0],
        USAGE_PALETTE_LIGHT[1],
        USAGE_PALETTE_LIGHT[2],
        USAGE_PALETTE_LIGHT[3],
        USAGE_PALETTE_LIGHT[4],
      ],
      borderWidth: 0,
    },
  ],
...
/** The palette a demo should use when it builds a chart of its own. */
export const chartPalette = {
  light: USAGE_PALETTE_LIGHT,
  dark: USAGE_PALETTE_DARK,
} as const;
...
/** Copy for `<lia-welcome-panel>`, kept here so the page stays declarative. */
