...
/** One value through one renderer — `renderCell` only needs a throwaway row and column. */
function cellDemo(renderer: CellRenderer, value: unknown, format?: CellFormatOptions): unknown {
  return renderCell({ value }, { key: 'value', label: 'Value', renderer }, { format });
}
      ...
      example(
        ...
        html`<div class="d-flex flex-wrap gap-4 align-items-start">
          ...
          <div>
            ...
            ${cellDemo('domainWithSan', {
              domain: 'svc.example.net',
              san: 'alt.example.net, svc.example.com',
            })}
          </div>
          <div>
            <p class="small text-body-secondary mb-1"><code>bytes</code> · <code>datetime</code></p>
            <div class="d-flex gap-3">
              <span>${cellDemo('bytes', 1_610_612_736)}</span>
              <span>${cellDemo('datetime', 0)}</span>
            </div>
          </div>
        </div>`,
        dedent`
          // The table renders every cell through one pure function — call it yourself
          // for a hand-written table:
          import { renderCell } from '@liasoft/lit-ui';

          html\`<td>\${renderCell(row, column, { format })}</td>\`;

          // A one-off value only needs a throwaway row and column:
          renderCell({ value: 1610612736 }, { key: 'value', label: 'Size', renderer: 'bytes' });
        `
      ),
      example(
        'Formatting options',
          ...
          <table class="table table-sm mb-0">
            ...
            <tbody>
              <tr>
                ...
                <td>${cellDemo('boolean', true)} ${cellDemo('boolean', false)}</td>
                <td>
                  ${cellDemo('boolean', true, {
                    trueLabel: 'Ja',
                    trueIcon: 'fa-solid fa-thumbs-up',
                    trueVariant: 'primary',
                  })}
                  ${cellDemo('boolean', false, {
                    falseLabel: 'Nein',
                    falseIcon: 'fa-solid fa-thumbs-down',
                  })}
                </td>
              </tr>
              <tr>
                <td><code>0</code> (datetime)</td>
                <td>${cellDemo('datetime', 0)}</td>
                <td>${cellDemo('datetime', 0, { neverText: 'noch nie' })}</td>
              </tr>
              <tr>
                <td><code>null</code></td>
                <td>${cellDemo('text', null)}</td>
                <td>${cellDemo('text', null, { emptyText: '(not set)' })}</td>
              </tr>
            </tbody>
          </table>
        </div>`,
