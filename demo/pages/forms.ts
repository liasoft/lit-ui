...
const REPLACERS: ReplacerToken[] = [
  { token: 'FIRSTNAME', description: 'Given name of the recipient', example: 'Amara' },
  { token: 'LASTNAME', description: 'Family name of the recipient', example: 'Okonkwo' },
  { token: 'ORGANISATION', description: 'Name of the organisation', example: 'Northwind Collective' },
  { token: 'INVITE_URL', description: 'One-time link that completes the invitation' },
  { token: 'EXPIRES_AT', description: 'When the link stops working', example: '3 July 2026, 09:30' },
  { token: 'SUPPORT_EMAIL', description: 'Reply-to address', example: 'help@example.org' },
  { token: 'INTERNAL_ID', description: 'Never rendered', visible: false },
];

const NOTIFICATION_FORM: FormDefinition = {
  id: 'notification-template',
  title: 'Invitation email',
  icon: 'fa-solid fa-envelope',
  description: 'Sent whenever someone is invited to the organisation.',
  sections: {
    main: {
      fields: {
        subject: {
          type: 'text',
          label: 'Subject',
          value: 'You have been invited to {ORGANISATION}',
          mandatory: true,
        },
        body: {
          type: 'textarea',
          ...
          value: [
            'Hello {FIRSTNAME},',
            '',
            'You have been invited to join {ORGANISATION}.',
            'Open {INVITE_URL} before {EXPIRES_AT} to set your password.',
            '',
  ...
  buttons: [{ label: 'Save the template', type: 'submit', variant: 'primary' }],
};
...
/** A form page: heading, note, placeholders and the form itself. */
@customElement('demo-form-page')
export class DemoFormPage extends LiaElement {
  @state() private values?: FormValues;
    ...
    return html`
      <div class="border rounded-3 overflow-hidden">
        <lia-form-page
          title="Invitation email"
          icon="fa-solid fa-envelope"
          description="What a new member receives the moment they are invited."
          back-url="#/forms/form"
          back-label="All templates"
          content-class="p-3 p-lg-4"
          submit-label="Save the template"
          .definition=${NOTIFICATION_FORM}
          .note=${{
            title: 'Placeholders are substituted at send time',
            text: 'Anything in braces is replaced before the message leaves the queue. Unknown placeholders are left untouched so a typo is visible in the preview.',
            variant: 'info',
          }}
          .replacers=${REPLACERS}
          .alerts=${[
            {
              id: 'draft',
              variant: 'secondary',
              message: 'This template has unsaved changes from 4 July.',
              dismissible: true,
            },
          ]}
          .rules=${{ subject: required('The subject cannot be empty.') }}
          @lia-submit=${(event: CustomEvent<FormSubmitDetail>) => {
            this.values = event.detail.values;
      ...
      ${valueDump(this.values, { title: 'Saved template', empty: 'Save it to see the values.' })}
    `;
  }
}
...
function renderPage(): TemplateResult {
  return html`
    ${section(
      'Form page',
      'Everything a create/edit screen needs around the form: a heading with a back link, an alert region, an explanatory note, the placeholder reference and the form.',
      example('Live', html`<demo-form-page></demo-form-page>`, dedent`
        html\`<lia-form-page
          title="Invitation email"
          icon="fa-solid fa-envelope"
          back-url="/templates"
          submit-label="Save the template"
          .definition=\${definition}
          .values=\${record}                 // prefill without touching the definition
          .errors=\${serverErrors}
          .rules=\${rules}
          .alerts=\${alerts}                 // AlertMessage[], dismissible
          .note=\${{ title: 'Placeholders', text: '…', variant: 'info' }}
          .replacers=\${tokens}              // the {PLACEHOLDER} reference table
          @lia-submit=\${(e: CustomEvent) => save(e.detail.values)}
        ></lia-form-page>\`;
      `, { bodyClass: 'p-0' })
    )}
    ${section('Form note', 'The panel above a form that explains what the form is for.', [
      example(
        'Variants',
        html`<div class="vstack gap-3">
          <lia-form-note
            title="Placeholders are substituted at send time"
            text="Anything in braces is replaced before the message leaves the queue."
            variant="info"
          ></lia-form-note>
          <lia-form-note
            variant="warning"
            icon="fa-solid fa-triangle-exclamation"
            text="Changing the home region <strong>recreates</strong> every volume."
            rich
          ></lia-form-note>
          <lia-form-note
            variant="secondary"
            no-icon
            dismissible
            text="You can dismiss this one."
            note-id="dismissible-note"
          ></lia-form-note>
        </div>`,
        dedent`
          html\`<lia-form-note title="…" text="…" variant="info"></lia-form-note>\`;

          // Or as data on a form page:
          .note=\${{ title: 'Placeholders', text: '…', variant: 'info', rich: true }}
        `
      ),
    ])}
    ${section('Placeholder reference', 'The table that tells an author which tokens exist — with a copy button on each.', [
      example(
        'Tokens',
        html`<lia-replacer-hint
          .tokens=${REPLACERS}
          title="Available placeholders"
          description="Click a token to copy it, then paste it into the subject or the body."
        ></lia-replacer-hint>`,
        dedent`
          const tokens: ReplacerToken[] = [
            { token: 'FIRSTNAME', description: 'Given name of the recipient', example: 'Amara' },
            { token: 'INVITE_URL', description: 'One-time link that completes the invitation' },
            { token: 'INTERNAL_ID', description: 'Never rendered', visible: false },
          ];

          html\`<lia-replacer-hint .tokens=\${tokens} prefix="{" suffix="}"></lia-replacer-hint>\`;
        `
      ),
      example(
        'Other delimiters, and empty',
        split(
          html`<lia-replacer-hint
            .tokens=${REPLACERS.slice(0, 3)}
            prefix="%"
            suffix="%"
            title="Shell-style tokens"
          ></lia-replacer-hint>`,
          html`<lia-replacer-hint
            .tokens=${[]}
            title="No placeholders"
            empty-message="This template takes no substitutions."
          ></lia-replacer-hint>`
        ),
        undefined
      ),
...
export const formsRoutes: DemoRoute[] = [
  ...
  {
    path: '/forms/page',
    title: 'Form page & notes',
    group: 'forms',
    icon: 'fa-solid fa-file-lines',
    description: 'A whole edit screen: heading, alerts, note and placeholders.',
    keywords: ['form page', 'note', 'replacer', 'placeholder', 'template'],
    render: renderPage,
  },
];
